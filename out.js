(() => {
  ...
  var import_gsap = __require("gsap");
...
<rect width="24" height="24" fill="white"/>
  ...
  function openDB(name5, version5, { blocked, upgrade, blocking, terminated } = {}) {
    ...
    openPromise.then((db2) => {
      ...
      if (blocking) {
        db2.addEventListener("versionchange", (event2) => blocking(event2.oldVersion, event2.newVersion, event2));
      }
    }).catch(() => {
    });
    return openPromise;
  ...
  var CookiePersistence = class {
    ...
    _addListener(key, listener) {
      ...
      if (window.cookieStore) {
        const cb = ((event2) => {
          const changedCookie = event2.changed.find((change) => change.name === name5);
          if (changedCookie) {
            listener(changedCookie.value);
          }
          const deletedCookie = event2.deleted.find((change) => change.name === name5);
          if (deletedCookie) {
            listener(null);
          }
        });
  ...
  var AbstractPopupRedirectOperation = class {
    ...
    async onAuthEvent(event2) {
      const { urlResponse, sessionId, postBody, tenantId, error, type } = event2;
      if (error) {
        this.reject(error);
        return;
      }
  ...
  var AuthEventManager = class {
    ...
    sendToConsumer(event2, consumer) {
      if (event2.error && !isNullRedirectEvent(event2)) {
        const code = event2.error.code?.split("auth/")[1] || "internal-error";
...
${this.customData.serverResponse}`;
  ...
  var firebaseConfig = {
    ...
    appId: "1:620668510760:web:a9cd9a21e5ee4d2ea64b1d"
  };
  var db = null;
  var auth = null;
  var storage = null;
  ...
  try {
    ...
    try {
      db = (0, import_firestore.initializeFirestore)(app, {
        ...
        localCache: (0, import_firestore.persistentLocalCache)({ tabManager: (0, import_firestore.persistentMultipleTabManager)() })
      });
    } catch (e) {
    ...
    console.log("\u{1F525} Connected to Firebase Firestore and Storage");
  } catch (e) {
  ...
  function OBSOverlay({ matchId, tournamentId }) {
    ...
    if (!isCompleted && match?.status !== "SCHEDULED" /* SCHEDULED */) {
      mappedSets.push({
    ...
    const animateScoreBox = (key) => {
      ...
      import_gsap.gsap.fromTo(
        ...
        { scale: 1, backgroundColor: "rgba(26,86,219,0.55)", borderColor: "#4a7dfa" },
        {
          ...
          yoyo: true,
          repeat: 1,
          onComplete: () => import_gsap.gsap.set(el, { scale: 1, clearProps: "backgroundColor,borderColor" })
        }
      );
    };
    const animatePointsBox = (el) => {
      if (!el) return;
      import_gsap.gsap.fromTo(
        el,
        { scale: 1, color: "#FFFFFF", textShadow: "none" },
        {
          ...
          yoyo: true,
          repeat: 1,
          onComplete: () => import_gsap.gsap.set(el, { scale: 1, clearProps: "all" })
        }
      );
    };
    const triggerSting = () => {
      ...
      import_gsap.gsap.set(layer, { display: "flex", opacity: 0 });
      import_gsap.gsap.set(rule, { width: "0%", opacity: 1 });
      import_gsap.gsap.set(body, { opacity: 0, scale: 0.58 });
      import_gsap.gsap.set([l1, l2], { opacity: 0, y: 5 });
      import_gsap.gsap.timeline({ onComplete: () => {
        stingActiveRef.current = false;
      } }).to(layer, { opacity: 1, duration: 0.28, ease: "power2.out" }).to(rule, { width: "90%", duration: 0.44, ease: "power3.inOut" }, "-=0.05").to(body, { opacity: 1, scale: 1, duration: 0.5, ease: "back.out(2.4)" }, "-=0.20").to(l1, { opacity: 1, y: 0, duration: 0.3, ease: "power2.out" }, "-=0.25").to(l2, { opacity: 1, y: 0, duration: 0.3, ease: "power2.out" }, "-=0.18").to({}, { duration: 2 }).to([body, l1, l2], { opacity: 0, y: -8, duration: 0.28, ease: "power2.in", stagger: 0.04 }).to(rule, { width: "0%", opacity: 0, duration: 0.28, ease: "power3.in" }, "-=0.20").to(layer, { opacity: 0, duration: 0.22, ease: "power2.in" }, "-=0.10").set(layer, { display: "none" });
    };
    (0, import_react.useEffect)(() => {
      document.fonts.ready.then(() => {
        if (stingTimerRef.current) clearInterval(stingTimerRef.current);
        stingTimerRef.current = setInterval(triggerSting, 6e4);
      });
      return () => {
        if (stingTimerRef.current) clearInterval(stingTimerRef.current);
      };
    }, [match?.score]);
    const isWinningSet = (team, setIdx) => {
      const s = mappedSets[setIdx];
      if (!s) return false;
      return team === "team1" ? s.team1 > s.team2 && setIdx < setsCount : s.team2 > s.team1 && setIdx < setsCount;
    };
    if (!match || match.status === "SCHEDULED" /* SCHEDULED */ || (match.status === "COMPLETED" /* COMPLETED */ || String(match.status).toUpperCase() === "FINISHED")) return null;
    const formatFirstNamesOnly = (fullName) => {
      if (!fullName) return "";
    ...
    const t1NameRaw = match.team1Name || "Team 1";
    const t1Name = formatFirstNamesOnly(t1NameRaw) || (match.team1Id ? "TEAM 1" : "TBD");
    const t2NameRaw = match.team2Name || "Team 2";
    const t2Name = formatFirstNamesOnly(t2NameRaw) || (match.team2Id ? "TEAM 2" : "TBD");
    const activeOverlay = match?.activeOverlay || "scoreboard";
    return /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.obsPage, children: [
      activeOverlay === "scoreboard" && /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.obsCard, ref: cardRef, children: [
        /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.obsBody, children: [
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: obs_default.obsLogoWrap, children: /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(MatchupLogo, { className: obs_default.obsLogoSvg }) }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.obsTeams, children: [
            /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.obsRow, children: [
              /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: obs_default.obsTname, children: t1Name }),
              /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.obsSets, children: [
                mappedSets.map((s, i) => /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(
                  "div",
                  {
                    ref: (el) => {
                      scoreRefs.current[`t1s${i}`] = el;
                    },
                    className: `${obs_default.obsSbox} ${isWinningSet("team1", i) ? obs_default.win : ""}`,
                    children: s.team1
                  },
                  i
                )),
                !isCompleted && /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: `${obs_default.obsSbox} ${obs_default.liveScoreValue}`, children: /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { ref: points1Ref, children: match.score?.p1Points === "0" ? "0" : match.score?.p1Points }) })
              ] })
            ] }),
            /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.obsRow, children: [
              /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: obs_default.obsTname, children: t2Name }),
              /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.obsSets, children: [
                mappedSets.map((s, i) => /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(
                  "div",
                  {
                    ...
                    className: `${obs_default.obsSbox} ${isWinningSet("team2", i) ? obs_default.win : ""}`,
                    children: s.team2
                  },
                  i
                )),
                !isCompleted && /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: `${obs_default.obsSbox} ${obs_default.liveScoreValue}`, children: /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { ref: points2Ref, children: match.score?.p2Points === "0" ? "0" : match.score?.p2Points }) })
              ] })
            ] })
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.stingLayer, ref: stingLayerRef, children: [
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: obs_default.stingRule, ref: stingRuleRef }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.stingBody, ref: stingBodyRef, children: [
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(MatchupLogo, { className: obs_default.stingLogo }),
            /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: obs_default.stingWords, children: [
              /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: obs_default.stingTaglineTop, ref: stingLine1Ref, children: "THE SPECTATORS" }),
              /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: obs_default.stingTaglineBot, ref: stingLine2Ref, children: "SPORTS PLATFORM" })
            ] })
          ] })
        ] })
      ] }),
      activeOverlay === "team_vs_team" && /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "absolute inset-0 flex items-center justify-center p-8 bg-black/80 backdrop-blur-md z-50", children: /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "text-center w-full max-w-4xl", children: [
        /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("h2", { className: "text-4xl font-black text-brand tracking-[0.2em] mb-4 uppercase", children: "Matchup" }),
        /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "flex items-center justify-between mt-12 bg-white/5 border border-white/10 rounded-3xl p-12", children: [
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "flex-1 text-center", children: [
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-5xl font-black text-white tracking-widest", children: t1Name }),
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-xl text-content-muted mt-2 font-mono uppercase", children: "Team 1" })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-3xl font-black text-white/40 italic px-8", children: "VS" }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "flex-1 text-center", children: [
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-5xl font-black text-white tracking-widest", children: t2Name }),
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-xl text-content-muted mt-2 font-mono uppercase", children: "Team 2" })
          ] })
        ] })
      ] }) }),
      activeOverlay === "player_profiles" && /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "absolute inset-0 flex items-center justify-center p-8 bg-black/80 backdrop-blur-md z-50", children: /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "w-full max-w-6xl", children: [
        /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("h2", { className: "text-3xl font-black text-center text-brand tracking-[0.2em] mb-8 uppercase", children: "Player Profiles" }),
        /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "grid grid-cols-2 lg:grid-cols-4 gap-6", children: [...match?.team1PlayerNames?.split(" & ") || ["P1", "P2"], ...match?.team2PlayerNames?.split(" & ") || ["P3", "P4"]].map((p, i) => /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "bg-white/5 border border-white/10 rounded-2xl p-6 text-center flex flex-col items-center", children: [
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "w-24 h-24 rounded-full bg-white/10 flex items-center justify-center mb-4" }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-xl font-black text-white uppercase tracking-wider", children: p.trim() }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-sm font-mono text-content-muted mt-2", children: "Win %: 0.00" }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-xs font-bold text-brand uppercase mt-1", children: "Category: Pro" })
        ] }, i)) })
      ] }) }),
      activeOverlay === "winner_result" && /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "absolute inset-0 flex items-center justify-center bg-black z-50 p-4", children: /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "w-[450px] aspect-[9/20] bg-surface-ground border border-white/10 rounded-[40px] p-8 flex flex-col items-center justify-center relative overflow-hidden", children: [
        /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "absolute inset-0 bg-brand/20 blur-[100px] pointer-events-none rounded-full scale-150" }),
        /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("h2", { className: "text-2xl font-black text-white tracking-[0.3em] uppercase mb-12 z-10 text-center", children: "Final Result" }),
        /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "w-full space-y-6 z-10", children: [
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: `p-6 rounded-2xl border flex flex-col items-center ${match?.winnerTeamId === match?.team1Id ? "bg-brand/20 border-brand" : "bg-white/5 border-white/10"}`, children: [
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: "text-3xl font-black text-white text-center uppercase tracking-wider", children: t1Name }),
            match?.winnerTeamId === match?.team1Id && /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: "text-sm text-brand font-bold uppercase mt-2", children: "Winner" })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "text-center font-black text-white/30 italic", children: "VS" }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: `p-6 rounded-2xl border flex flex-col items-center ${match?.winnerTeamId === match?.team2Id ? "bg-brand/20 border-brand" : "bg-white/5 border-white/10"}`, children: [
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: "text-3xl font-black text-white text-center uppercase tracking-wider", children: t2Name }),
            match?.winnerTeamId === match?.team2Id && /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: "text-sm text-brand font-bold uppercase mt-2", children: "Winner" })
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "mt-12 flex gap-4 z-10 font-mono text-xl", children: mappedSets.map((s, i) => /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "flex flex-col items-center bg-black/50 rounded-lg p-3 border border-white/5", children: [
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: `font-bold ${isWinningSet("team1", i) ? "text-white" : "text-content-muted"}`, children: s.team1 }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "w-4 h-[1px] bg-white/10 my-1" }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: `font-bold ${isWinningSet("team2", i) ? "text-white" : "text-content-muted"}`, children: s.team2 })
        ] }, i)) })
      ] }) })
    ] });
  }
